...
def change_floor(level: number):
    global currFloor
    if level == 1:
        if currFloor < maxFloor:
            currFloor += 1
            mySprite.set_position(11, mySprite.y)
            mySprite2.set_position(11, mySprite2.y)
            mySprite.say_text(currFloor)
    elif currFloor > 1:
        currFloor += -1
        mySprite.set_position(scene.screen_width() - 11, mySprite.y)
        mySprite2.set_position(scene.screen_width() - 11, mySprite2.y)
        mySprite.say_text(currFloor)
    if currFloor == 1:
...
mySprite.set_position(40, 79)
mySprite2.set_position(30, 79)
mySprite2.follow(mySprite)
controller.move_sprite(mySprite)

def on_update_interval():
    if mySprite.x >= scene.screen_width() - 10 and currFloor <= maxFloor:
        change_floor(1)
    if mySprite.x <= 10 and currFloor > 1:
        change_floor(-1)
